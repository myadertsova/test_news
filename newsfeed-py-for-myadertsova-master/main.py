...
def get_list():
    ...
    for news_item in filter(news_filter, news['news']):
        ...
        correct_news.append(news_item)

    return jsonify({"news": correct_news, 'totalResults': len(correct_news)})


def news_filter(news):
    return True if datetime.now().isoformat() > news['publishedAt'] and not news['isDeleted'] else False
